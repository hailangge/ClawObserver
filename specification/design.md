...
- operator runs a single install script from the repo
- the install script renders user-level `systemd` units and an env file into the operator's home directory
...
- install reruns are idempotent and preserve an existing env file by default
- rendered units always use the current repo path and user home substitutions
- install prints the local URL plus status and management commands
- uninstall stops/disables/removes the user units, runs `daemon-reload`, and preserves config/state unless destructive flags are explicitly selected

The install/uninstall scripts remain thin wrappers around the existing host-native Python plus user-level `systemd` model, not a new packaging system.
...
- If deterministic Quaternius provenance is unavailable or too risky, the renderer should keep the current Kenney-local geometry pipeline and push style through chunkier scales, darker midnight-navy shell materials, steel/cobalt furniture recolors, cyan screen/emissive trims, sparse muted-orange accents, rounded helper primitives, and denser prop composition instead of changing the asset provenance baseline.
...
- Temporary character previews should use a separate renderer-local seam and separate provenance metadata from the furniture catalog so preview-only seated placeholders can be added or removed without mutating the underlying desk/monitor asset contract. The current approved preview exception is a repo-local Poly Pizza `Hyper Casual Character` CC BY 3.0 import with explicit attribution/provenance; it remains preview-only and must not be described as the final avatar direction.
- Fixed workstation orientation is part of the renderer contract: desk fronts and monitor screen fronts must read coherently from the camera view, preview or future seated characters must be placed behind the desk and face the monitor rather than its back, and floating label plates keep their independent camera-facing billboard yaw.
- Monitor styling is part of the renderer contract: desks should expose a readable screen plane with cyan/electric-blue edge treatment and still-visible keyboard/mouse props, but the effect must stay lightweight enough to preserve `frameloop="demand"` and the existing idle perf guard.
- Browser smoke metadata should continue to assert the high-level scene contract and should be extended when practical to capture the approved palette/orientation/monitor-style profile in addition to the existing fit, provenance, opacity, label-clearance, and performance assertions.
   - active sessions
   - total sessions
...
- live refresh must be bounded and low-churn rather than visibly page-reloading; automatic polls update the existing realtime DOM/scene in place, while manual refresh can still force a full page fetch cycle
...
- In the WorkAdventure primary renderer, work-area overlays should span the real usable workspace on the approved custom background instead of only tiny hotspot rectangles, while lounge/rest bounds remain tighter and correctness-enforced.
- WorkAdventure marker/nameplate legibility is part of the contract: person chips and compact single-line nameplates should be modestly enlarged relative to the current dense-desk pass without reintroducing overlap or clipping. The renderer contract for current desk/lounge labels is `agent-name(taskCount)` on one line, with the same text rule applied across work and rest placements unless a truthful degraded fallback is required.
...
5. installation through the repo script
6. configuration file location and key env settings
7. installed unit names plus service-management and timer-management commands
8. manual capture and history-seeding commands
9. uninstall behavior, including safe defaults and explicit destructive flags
10. loopback default access behavior and how to override it intentionally
11. runtime adapter precedence and fallback behavior
12. archive cadence and daily-last-record semantics
13. development and validation commands
14. operator-visible data paths plus asset/provenance notes
15. troubleshooting focused on the local host-native install path
...
- Automatic realtime polling must reuse the existing page shell and the existing `#realtime-r3f-scene-mount` React root after the first successful render; normal background refreshes should patch metric/table HTML and rerender the embedded scene without calling a full-page `innerHTML` replacement or `unmountRealtimeScene()`.
- Enforce a frontend-side minimum live refresh floor so pathological `refresh_seconds` values cannot create multi-second visible reload cadence or sustained main-thread churn.
- Browser smoke should validate at least two successive realtime polls and fail if the realtime scene mount node identity changes, if the same background/avatar assets are fetched repeatedly on steady-state refresh, or if post-settle animation frames exceed the idle guard after the page has stabilized.
...
### 6.4 WorkAdventure-style tile-map realtime track

The WorkAdventure-style tile-map renderer is the primary visual implementation for the current Realtime page direction. The existing React Three Fiber scene remains in the repository as an alternate / legacy 3D style and must not be deleted.

Design rules:
- Source official starter-map material from `workadventure/map-starter-kit` where practical, preserving upstream maps, tilesets, thumbnails, and license files under a repo-local asset directory.
- Source a small official WorkAdventure avatar/Woka resource subset from the WorkAdventure runtime where practical; if full runtime avatar composition is too heavy, bundle audited sprite/layer PNGs and render them deterministically as static/live-positioned people markers.
- Keep provenance machine-readable so later asset audits can identify upstream repository, commit, source paths, and license boundaries.
- Render the primary embedded Realtime scene through a lightweight browser surface that consumes the existing `AgentVisualState[]` and `SceneSummary` contract.
- Preserve the same live-data mapping seam used by the 3D scene: no new backend payload shape is required for this pass.
- Model the concepts the later integration needs: tiled office background, walkable/collision semantics, interaction zones, single-person desk labels/nameplates, and sprite/avatar resources with stable foot-anchor positions.
- Keep the alternate 3D renderer accessible from the prototype/legacy path or an explicit style switch so future 3D model work can resume without recovering deleted code.

Visual style rules:
- Prefer a cleaner WorkAdventure-inspired office palette: warm office floor, readable cyan/blue interaction overlays, status-colored desk/person badges, and high-contrast but non-neon labels.
- Single-person office desks should be visually distinct and labeled with concise hanging/nameplate-style tags tied to agent/session data.
- Interaction zones should read as affordances, not error overlays; zone labels must be legible in screenshot tests.

Validation rules:
- Build must remain deterministic.
- Browser smoke must cover the embedded Realtime page, the WorkAdventure-style surface, the retained 3D/prototype route, and screenshot evidence.
- The smoke output must report zero page errors, zero console errors, visible map surface, visible interaction zones, visible official avatar/person resources, and live-data-derived desk/nameplate labels.
