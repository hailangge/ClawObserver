...
- The active visual palette for the shared office scene must align with a deep navy / midnight-blue command-center environment, steel and cobalt-blue furniture, cyan / electric-blue emissive accents and screen edges, and only sparse muted-orange accents when useful; cream/white/tan toy colors must not dominate the scene.
- The chunky toy-office / Quaternius-inspired geometry direction remains approved, but the recolor target is the darker static-scene tech-command-center palette rather than a warm pastel office palette.
...
- A temporary non-final preview may seat selected desks with a repo-local attribution-documented character subset or temporary local avatar implementation to test readability and desk-facing composition, but it must be labeled preview-only, keep deterministic local provenance/license metadata for any retained third-party preview pack, remain optional to remove later, and must not change the live `AgentVisualState` contract or imply final avatar direction.
- The currently approved preview avatar exception is Poly Pizza `Hyper Casual Character` by `J-Toastie` under CC BY 3.0, imported repo-locally with explicit attribution/provenance and used only as a temporary seated office preview.
- WorkAdventure-style tile-map rendering is now the primary Realtime scene implementation for the current product direction, while the existing React Three Fiber office scene must remain available as an alternate / legacy 3D style that can be resumed later.
- The embedded Realtime page must render the WorkAdventure-style office surface from live `AgentVisualState` data, including live-derived people/avatar placements, single-person desk labels/badges, interaction zones, and status summary affordances.
- The WorkAdventure-style implementation must use official WorkAdventure map-starter-kit resources where license-compatible and clearly label them as WorkAdventure-map-only assets; it must not delete the production R3F scene or violate existing avatar preview/provenance contracts.
- The WorkAdventure-style sample must demonstrate the intended long-term 2D/tile approach: office tile map, collision/walkable layer concept, interaction zones, reusable transparent sprite/avatar resources, and a browser-rendered visual surface suitable for screenshot-based review.
- This pass must add official WorkAdventure avatar/Woka-style sprite resources, or a small audited subset of official WorkAdventure avatar composition layers if full runtime composition is too heavy, with deterministic repo-local provenance and license notes.
- Any copied WorkAdventure starter-kit tilesets, maps, thumbnails, avatar resources, or license files must remain repo-local, provenance-documented with upstream repository/commit/source path, and accompanied by asset-license notes that these official assets are only for WorkAdventure SaaS/self-hosted WorkAdventure maps unless a later license review approves broader reuse.

- The approved custom static background for the current WorkAdventure primary renderer is `/home/massena/Downloads/Generated Image May 12, 2026 - 2_20PM.jpg` / workspace copy `tmp/user-images/generated-office-2026-05-12-1420.jpg`; it must be imported repo-locally with provenance metadata before use.
- The custom background integration must preserve stable overlay geometry: busy/working agents in the work area, idle/non-working agents in the rest/lounge area, no desk watermark clutter, no incorrect occlusion, no nameplate overlap, and no selected-agent detail panel pushing the map out of alignment.
- The approved work-area overlays on that background must cover the actual broader visible workspace, not only three small islands; desk-bay and central-work overlays should span the usable office floor while preserving the existing lounge/rest correctness contract.
- Agent nameplate text rendered on the WorkAdventure image must use a compact single-line `agent-name(taskCount)` format for both work markers and lounge/rest markers unless a degraded fallback truly cannot provide agent identity, and the type should be modestly larger and more readable than the prior dense-desk two-line cards without reintroducing overlap.
- Browser smoke must include visual-regression-style geometry checks for the custom map: image loads successfully, rendered background aspect ratio matches expected `1264x848`, occupied markers stay inside approved work/rest zones, compact labels render with the expected `agent-name(taskCount)` text, nameplate type stays at or above the approved minimum readable size, labels remain non-overlapping, and the retained 3D route still renders.
- Realtime page refresh behavior must stay operational rather than visibly game-like: manual refresh remains available, but automatic live refresh must not cause visible page rebuilds, React scene remounts, repeated asset fetches, or a cadence that looks like the page is reloading every few seconds.
- The Realtime page must enforce a sane minimum auto-refresh interval even when the payload advertises an overly small `refresh_seconds` value, and automatic polls must update the existing mounted scene in place rather than tearing down and rebuilding the scene DOM.
- Browser smoke and automated guards must detect rapid refresh/remount regressions by failing on too-small scheduled refresh cadence, excessive post-settle animation frames, or repeated background/Woka asset fetches across successive realtime polls.

- Realtime WorkAdventure rendering must place every renderer-facing agent from `AgentVisualState[]`; it must not silently drop agents because work/rest anchor capacity is smaller than the live payload.
- Work/rest placement is a hard correctness rule: busy/error agents must render only in the working area; idle/offline/waiting agents must render only in the rest/lounge area. Smoke tests must fail if any idle/offline agent is rendered in a work marker.
- The standard smoke fixture must include enough agents to exercise overflow capacity beyond the original 4-agent demo and must assert rendered agent count equals input agent count.
- All fixed desks must visually face the camera/screen side consistently: desk fronts and monitor screens should face the camera together, future seated/standing characters must be able to occupy the desk-back side while facing the camera, and the label plates must retain independent camera-facing yaw behavior.
- Desk monitor treatment should remain compact and lightweight but should clearly read as a real workstation through visible screen planes, cyan edge or bezel glow, and still-visible keyboard/mouse props without cluttering the desk or violating the frameloop/performance guard.
...
- the central Realtime scene should remain operational and readable, but it should no longer feel bare; it should present a stylized Q-version/toy-like office treatment with rounded/chunky forms, a navy/cobalt/cyan command-center palette, matte materials, restrained emissive edges, and clearer prop storytelling
...
4. The repository must also ship a user-facing uninstall script for the same host-native user-level `systemd` deployment path.
5. The install script must be safe and idempotent: reruns must preserve operator-edited env configuration by default, rerender unit files with the current repo path, daemon-reload the user manager, and enable/start the web service plus archive timer.
6. The uninstall script must safely stop, disable, and remove the user units, daemon-reload afterwards, and preserve config/state by default unless the operator explicitly opts into destructive removal flags.
7. The repository README must be suitable as an operator manual and must explain prerequisites, quick start, installation, configuration, service/timer management, manual capture, uninstall, runtime adapter behavior, archive cadence semantics, development, validation, data paths, provenance notes, and troubleshooting.
8. The repository’s product/specification documents must live under `specification/` rather than relying on root-level spec files.
